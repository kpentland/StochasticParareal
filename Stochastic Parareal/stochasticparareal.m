...
for s = 1:sims
    ...
    for k = 2:N
        ...
        if n == 1
            SIG = repmat(diag(ones(1,n)),[1 1 N+1]);     %for 1D system there are no dimensions to correlate
        else                                             %for systems of ODEs we need the correlations between solutions in each dimension
            for i = I:N                                  %can parallelise this loop for high dimensional systems
        ...
        parfor II = 1:(chunks*m)+1
            ...
            if II == (chunks*m) + 1
                ...
                if II <= chunks
                    ...
                    %otherwise we sample from a multivar normal distribution
                else
                    if sample_rule == 1
